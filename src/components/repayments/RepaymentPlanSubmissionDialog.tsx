...
interface Props {
  ...
  onSubmit: (payload: { recipient: "investor" | "platform"; investment: string | null; project: string; notes: string; installments: RepaymentPlanInstallment[] }) => void;
}
...
const RepaymentPlanSubmissionDialog = ({ open, investments, plan, pending, onOpenChange, onSubmit }: Props) => {
  ...
  const selected = useMemo<EligibleRepaymentInvestment | undefined>(() => {
    if (plan) return {
      id: plan.recipient === "platform" ? `platform:${plan.project_id}` : plan.investment!,
  ...
  useEffect(() => {
    if (!open) return;
    const initial = plan
      ? (plan.recipient === "platform" ? `platform:${plan.project_id}` : plan.investment || "")
      : investments[0]?.id || "";
    setInvestmentId(initial);
    setNotes(plan?.notes || "");
    setInstallments(plan?.installments.map((item) => ({
      amount: item.amount,
      recipient: plan.recipient,
  ...
  const submit = (event: FormEvent) => {
    ...
    if (!investmentId || !selected || !totalsMatch || installments.some((item) => !item.amount || !item.scheduled_date)) return;
    onSubmit({
      recipient: selected.recipient,
      investment: selected.recipient === "investor" ? selected.id : null,
      project: selected.project_id,
      notes: notes.trim(),
      installments: installments.map((item) => ({ ...item, recipient: selected.recipient })),
    });
  };
      ...
      <DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-3xl">
        ...
        {!plan && investments.length === 0 ? (
          ...
          <div className="space-y-2">
            <Label htmlFor="repayment-target">{t("repaymentPlanSubmission.selectRepayment")}</Label>
            {plan ? <div className="rounded-xl border bg-muted/30 p-4"><p className="font-semibold">{plan.recipient === "platform" ? t("repaymentPlanSubmission.platformFeeTitle") : plan.investor_name}</p><p className="text-sm text-muted-foreground">{plan.project_title}</p></div> : (
              <Select value={investmentId} onValueChange={(value) => { setInvestmentId(value); const option = investments.find((item) => item.id === value); setInstallments([emptyInstallment(option?.earliest_repayment_date || "")]); }}>
                <SelectTrigger id="repayment-target"><SelectValue placeholder={t("repaymentPlanSubmission.selectRepaymentPlaceholder")} /></SelectTrigger>
                <SelectContent>{investments.map((item) => <SelectItem key={item.id} value={item.id}>{item.recipient === "platform" ? t("repaymentPlanSubmission.platformFeeTitle") : item.investor_name} · {item.project_title} · {formatCurrency(item.obligation_total)}</SelectItem>)}</SelectContent>
              </Select>
            )}
          </div>

          {selected && <div className={`grid gap-3 rounded-xl bg-muted/40 p-4 text-sm ${selected.recipient === "platform" ? "grid-cols-2" : "grid-cols-3"}`}>
            <div><p className="text-muted-foreground">{t(selected.recipient === "platform" ? "repaymentPlanSubmission.projectFunding" : "repaymentPlan.principal")}</p><p className="font-semibold">{formatCurrency(selected.principal)}</p></div>
            {selected.recipient === "investor" && <div><p className="text-muted-foreground">{t("repaymentPlan.expectedReturn")}</p><p className="font-semibold">{formatCurrency(selected.expected_return)}</p></div>}
          ...
          <DialogFooter><Button type="button" variant="outline" disabled={pending} onClick={() => onOpenChange(false)}>{t("common.cancel")}</Button><Button type="submit" disabled={pending || !investmentId || !totalsMatch}><Send className="h-4 w-4" />{pending ? t("common.saving") : t("repaymentPlanSubmission.submit")}</Button></DialogFooter>
        </form>}
      </DialogContent>
    </Dialog>
  );
