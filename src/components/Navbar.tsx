...
const Navbar = () => {
  ...
  const links = navLinks.map((link) => <Link key={link.href} to={link.href} onClick={() => setOpen(false)} className={`rounded-lg px-3 py-2 text-sm font-medium transition-colors hover:bg-muted ${location.pathname === link.href ? "text-primary" : "text-muted-foreground"}`}>{t(link.key)}</Link>);
  ...
  return <header className="sticky top-0 z-50 border-b border-border bg-card/80 backdrop-blur-lg">
    ...
    {open && <div id="mobile-navigation" className="border-t border-border bg-card md:hidden"><nav className="container flex flex-col gap-1 py-4" aria-label={t("nav.menu")}>
      {links}<LanguageSwitcher className="mt-2 self-start" />
