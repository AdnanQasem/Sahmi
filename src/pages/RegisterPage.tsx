          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <div>
              ...
              <select
                id="user_type"
                value={userType}
                onChange={(event) => setUserType(event.target.value as "investor" | "entrepreneur")}
                className="mt-1.5 h-10 w-full rounded-lg border border-input bg-background px-3 text-sm text-foreground"
              >
                <option value="investor">Investor</option>
                <option value="entrepreneur">Entrepreneur</option>
              </select>
              {fieldErrors.user_type && <p className="mt-1 text-xs text-destructive">{fieldErrors.user_type}</p>}
            </div>
            <div>
